@testitem "HPACK: huffman encoding" begin
    ...
    @testset "Basic encoding roundtrip" begin
        ...
        for s in test_strings
            ...
            encoded = HTTP2.huffman_encode(raw_data)
            decoded = HTTP2.huffman_decode(encoded)
    ...
    @testset "Empty string" begin
        empty = UInt8[]
        encoded = HTTP2.huffman_encode(empty)
    ...
    @testset "Single characters" begin
        for c in ['a', 'A', '0', ' ', '/', ':']
            ...
            encoded = HTTP2.huffman_encode(data)
            decoded = HTTP2.huffman_decode(encoded)
...
@testitem "HPACK: huffman encoded length" begin
    ...
    @testset "Length calculation matches actual encoding" begin
        ...
        for s in test_strings
            raw_data = Vector{UInt8}(s)
            predicted_len = HTTP2.huffman_encoded_length(raw_data)
            actual_encoded = HTTP2.huffman_encode(raw_data)
            @test predicted_len == length(actual_encoded)
        end
    end

    @testset "Empty string" begin
        @test HTTP2.huffman_encoded_length(UInt8[]) == 0
    end
end
...
@testitem "HPACK: integer encoding" begin
    ...
    @testset "Small values (fit in prefix)" begin
        for prefix_bits in [5, 6, 7]
            ...
            for value in 0:(max_prefix - 1)
                encoded = HTTP2.encode_integer(value, prefix_bits)
    ...
    @testset "Large values (multi-byte encoding)" begin
        ...
        for (value, prefix_bits) in test_cases
            encoded = HTTP2.encode_integer(value, prefix_bits)
            decoded, offset = HTTP2.decode_integer(encoded, 1, prefix_bits)
            @test decoded == value
    ...
    @testset "Roundtrip for various prefix sizes" begin
        for prefix_bits in [4, 5, 6, 7]
            for value in [0, 1, 10, 100, 1000, 10000]
                encoded = HTTP2.encode_integer(value, prefix_bits)
                decoded, _ = HTTP2.decode_integer(encoded, 1, prefix_bits)
                @test decoded == value
            end
        end
    end
...
@testitem "HPACK: string encoding" begin
    ...
    @testset "Raw encoding (no Huffman)" begin
        ...
        for s in test_strings
            encoded = HTTP2.encode_string(s; huffman=false)
            @test (encoded[1] & 0x80) == 0
            decoded_str, _ = HTTP2.decode_string(encoded, 1)
            @test decoded_str == s
        end
    end
    ...
    @testset "Huffman encoding" begin
        ...
        for s in test_strings
            encoded = HTTP2.encode_string(s; huffman=true)
            raw_len = length(s)
            huff_encoded = HTTP2.huffman_encode(Vector{UInt8}(s))
    ...
    @testset "Empty string" begin
        encoded_raw = HTTP2.encode_string(""; huffman=false)
        @test encoded_raw == UInt8[0x00]

        encoded_huff = HTTP2.encode_string(""; huffman=true)
        @test encoded_huff == UInt8[0x00]

        decoded, _ = HTTP2.decode_string(encoded_raw, 1)
        @test decoded == ""
    end

    @testset "Huffman auto-fallback" begin
        short_string = "ab"
        encoded = HTTP2.encode_string(short_string; huffman=true)
        decoded, _ = HTTP2.decode_string(encoded, 1)
...
@testitem "HPACK: dynamic table" begin
    ...
    @testset "Basic operations" begin
        table = HTTP2.DynamicTable(4096)
        @test isempty(table.entries)
        @test table.size == 0
        @test table.max_size == 4096

        HTTP2.add!(table, "custom-header", "custom-value")
        @test length(table.entries) == 1
        @test table.entries[1] == ("custom-header", "custom-value")
        @test table.size == HTTP2.entry_size("custom-header", "custom-value")
    end

    @testset "Entry eviction" begin
        table = HTTP2.DynamicTable(100)

        HTTP2.add!(table, "header1", "value1")
        HTTP2.add!(table, "header2", "value2")
        HTTP2.add!(table, "header3", "value3")
    ...
    @testset "Get entry spanning static and dynamic" begin
        table = HTTP2.DynamicTable(4096)
        HTTP2.add!(table, "x-custom", "test")

        @test HTTP2.get_entry(table, 1) == (":authority", "")
        @test HTTP2.get_entry(table, 2) == (":method", "GET")
        @test HTTP2.get_entry(table, 3) == (":method", "POST")

        @test HTTP2.get_entry(table, 62) == ("x-custom", "test")
    end
end
...
@testitem "HPACK: encoder/decoder" begin
    ...
    @testset "Basic header encoding/decoding" begin
        encoder = HTTP2.HPACKEncoder(4096)
        decoder = HTTP2.HPACKDecoder(4096)
        ...
        encoded = HTTP2.encode_headers(encoder, headers)
    ...
    @testset "Indexed header field" begin
        ...
        encoded = HTTP2.encode_header(encoder, ":method", "GET")
        @test (encoded[1] & 0x80) != 0
    ...
    @testset "Literal header with indexing" begin
        ...
        encoded = HTTP2.encode_header(encoder, "x-custom", "value"; indexing=:incremental)
        decoded = HTTP2.decode_headers(decoder, encoded)
    ...
    @testset "Huffman encoding in headers" begin
        encoder = HTTP2.HPACKEncoder(4096; use_huffman=true)
        decoder = HTTP2.HPACKDecoder(4096)
    ...
    @testset "Multiple requests maintain state" begin
        ...
        encoded1 = HTTP2.encode_headers(encoder, headers1)
        decoded1 = HTTP2.decode_headers(decoder, encoded1)
        @test decoded1 == headers1

        headers2 = [("x-request-id", "req-002"), (":method", "POST")]
        encoded2 = HTTP2.encode_headers(encoder, headers2)
        decoded2 = HTTP2.decode_headers(decoder, encoded2)
...
@testitem "HPACK: entry size calculation" begin
    ...
    @test HTTP2.entry_size("name", "value") == 4 + 5 + 32
    @test HTTP2.entry_size("", "") == 0 + 0 + 32
    @test HTTP2.entry_size("content-type", "application/grpc") == 12 + 16 + 32
end
...
@testitem "HPACK: static table lookup" begin
    ...
    index, exact = HTTP2.find_index(table, ":method", "GET")
    @test index == 2
    @test exact == true

    # Name-only match in static table
    index, exact = HTTP2.find_index(table, ":method", "OPTIONS")
    @test index > 0
    @test exact == false

    # Not in table
    index, exact = HTTP2.find_index(table, "x-nonexistent", "value")
    @test index == 0
    @test exact == false
end
