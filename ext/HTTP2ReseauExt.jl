...
# HTTP2.jl when both HTTP2 and Reseau.jl are loaded in the same
# environment:
#
...
#   HTTP2.reseau_h2_connect(address; kwargs...)                      -> Reseau.TLS.Conn
#
# All three pre-populate `alpn_protocols = HTTP2.ALPN_H2_PROTOCOLS`
# (`== ["h2"]`) unless the caller overrides it explicitly.
#
# Design note: the M5 pattern for OpenSSL.jl is a **mutator** —
# `HTTP2.set_alpn_h2!(ctx::OpenSSL.SSLContext)` modifies a mutable
...
# gap in OpenSSL.jl that blocked HTTP2.jl's `serve_connection!`
...
function _h2_kwargs(kwargs)
    ...
    if !haskey(kwdict, :alpn_protocols)
        kwdict[:alpn_protocols] = HTTP2.ALPN_H2_PROTOCOLS
    end
    return (; kwdict...)
end

function HTTP2.reseau_h2_server_config(; cert_file, key_file, kwargs...)
...
function HTTP2.reseau_h2_client_config(; kwargs...)
    return Reseau.TLS.Config(; _h2_kwargs(kwargs)...)
end

function HTTP2.reseau_h2_connect(address::AbstractString; kwargs...)
    return Reseau.TLS.connect(address; _h2_kwargs(kwargs)...)
end

end # module HTTP2ReseauExt
