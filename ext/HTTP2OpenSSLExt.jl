module HTTP2OpenSSLExt

# Milestone 5: optional OpenSSL.jl package extension.
#
# Provides the single method `HTTP2.set_alpn_h2!(::OpenSSL.SSLContext)`
...
# `ssl_set_alpn`.
#
# This extension loads automatically when both HTTP2 and OpenSSL are
# present in the same environment (Julia's `Base.get_extension`
# mechanism). When OpenSSL is not loaded, `HTTP2.set_alpn_h2!` exists
...
function HTTP2.set_alpn_h2!(ctx::OpenSSL.SSLContext,
